# Financial Modeling Asset Comparison Project

This repository contains an Excel-based asset comparison project covering multiple asset classes including:

- Gold
- Silver
- Crude Oil
- Natural Gas
- USD/INR
- Bank Nifty
- Copper
- Zinc
- Soya

## What This Project Includes

The Excel workbook includes:

- Historical asset price data
- Annual return comparison
- VAMI analysis
- Asset performance comparison
- Commodity and market index analysis

## Who This Is Useful For

This project is useful for:

- Finance students
- Financial modeling learners
- Equity research aspirants
- Investment banking aspirants
- Portfolio analysis learners
- Excel modeling practice

## Download the Excel File

You can download the Excel workbook from this repository:

`asset-comparison-data.xlsx`

## Learn Financial Modeling

If you want to learn how to build practical Excel-based financial models, valuation models, and investment analysis projects, you can explore Mentor Me Careers’ Financial Modeling Course:


## About Mentor Me Careers

Mentor Me Careers provides practical finance training programs in Financial Modeling, Equity Research, Investment Banking Operations, CFA, ACCA, and AML/KYC.

Author: [mentormecareers](https://mentormecareers.com)
Relvant course: [Financial Modeling](https://mentormecareers.com/financial-modeling-course-with-placement/)
